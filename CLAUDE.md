...
- **5.41** (`partially-resolved`): repo-wide `npx tsc --noEmit` had 29 pre-existing type errors across 5 files / subprojects (admin/, app/drive.tsx:335, lib/__tests__/routeBadges.test.ts, scripts/poi-import/lib/category-map.ts:27, scripts/precache-popular-routes.ts:434). `scripts/poi-import/lib/category-map.ts:27` resolved 2026-05-22 during GNIS arc (the `tags: [v]` typo was both a tsc error AND a runtime crash that took down every poi-import command on module-load since commit `e031eac` 2026-05-11; fix split the hot_spring/geyser rule into two static-tag rules). The other 4 files / 28 errors still open. Deferred to a dedicated cleanup arc — does not block Phase 1. Phase 1's own files (App.tsx, app/index.tsx, src/**) have **zero** tsc errors.
...
3. **Source importers** — all 5 done.
   ...
   - **`sources/gnis.ts`** ✅ **LIVE (full CA, 2026-05-22)** — Pulls USGS `DomesticNames/DomesticNames_CA_Text.zip` from `prd-tnm.s3.amazonaws.com` (USGS reorganized the bucket in late 2025; old `State/TXT_FORMAT/CA_Features_YYYYMMDD.zip` path is dead — filenames no longer carry dates, freshness comes from S3 `LastModified`). 30-day file cache (`cache/gnis/`). Minimal built-in ZIP extractor using `zlib.inflateRaw` (no extra deps). **Curator decision 2026-05-22: GNIS is the silent topographic label layer**, not a narration source — `significance_score=0.05` (recompute normalises to 5/100) keeps every row under all category floors (geology=60, nature=65, volcanic=60, hot_springs=60). Rows populate map labels + tap-to-explore queries on Hiking/City pages; never compete for narration queue slots. `confidence_score=0.8`, `verified=true`, `description=NULL`, `narrative_modes` left to the §15.10 trigger (nature/geology → `{soul}`, volcanic/hot_springs → `{local}` via Bucket C default). Allowlisted classes + slug mapping per curator spec: Summit/Range → geology; Crater/Lava → volcanic; Geyser/Hot Spring → hot_springs (USGS no longer emits these as standalone classes — captured via `Spring` class + name-filter `/\bhot\s+spring|\bgeyser/i` instead); Falls/Cape/Arch/Bay/Pillar/Lake/Island → nature. County filter strips `" County"` suffix from CLI input + does exact-match comparison (data column carries short names like "Mono"). First full CA run: **10,989 rows** (geology 5,970 / nature 4,918 / hot_springs 59 / volcanic 42) in 18.9s, 0 errors. Idempotent on re-run via `ON CONFLICT (source_type, source_id) WHERE merged_into IS NULL DO UPDATE`. Known minor wart: tags array has a duplicate token per row (e.g. `["lake","lake"]`) because spec tags + lowercased class name overlap — cosmetic, benign for queries.
...
cache/
  ...
  gnis/               DomesticNames_CA_Text.zip + *.meta.json (USGS S3 download, 30-day; old CA_Features_*.zip layout retired by USGS late 2025)
...
**Applied 2026-05-19 (G2-floors — per-category significance floors wired into live runtime):**
- 20260519000003 `category_significance_floors_seed_g2` — UPSERT seeds the per-category floors per addendum §2.2 with curator-tuned values. Final table state is 17 rows. Top-level (11): geology=60, nature=65, history=70, local_culture=70, architecture=90 (curator-bumped from addendum §2.2's placeholder 80; pushes the burden of sub-90 architecture surfacing onto `editorial_curated`), art=75, food_drink=0 (only via iconic_local override; per addendum §1.1/§8 food/drink is opt-in via Narrative Focus + iconic bar), engineering=70, viewpoint=65 (scenic viewpoints at slightly reduced floor — between nature's 65 and history's 70), hidden_gems=70, recreation=70. Sub rows mirror parents: volcanic=60, hot_springs=60, native_history=70, bridges=70, dams=70, mining=70. Only `legends` falls through to COALESCE-70. Idempotent via `ON CONFLICT (category) DO UPDATE`; supersedes the B1 seed at 20260518000002 for geology/nature (numeric values unchanged at 60/65; notes reframed under the per-category-floor policy).
- 20260519000004 `rpc_pois_floor_and_priority` — updates both `get_corridor_pois` and `get_nearby_pois` with three coordinated G2 changes: (1) LEFT JOIN `category_significance_floors` and apply per-category floors via `GREATEST(COALESCE(csf.significance_floor, 70), min_significance)` — the existing `min_significance` param is preserved but its semantic shifts from "the only floor" to "an ADDITIONAL floor on top of the per-category floor" (callers passing 0 let the per-category floor win); (2) `editorial_curated = TRUE` and `iconic_local = TRUE` BYPASS the floor in the WHERE clause's OR-chain — a curator-approved or strict-iconic-bar POI surfaces regardless of significance_score; (3) new `priority_tier text` column in RETURNS shape (`'curator'` / `'iconic'` / `'standard'`); ORDER BY promotes by tier first, then `significance_score DESC`, then the existing spatial sort (arc-length-along-route / ST_Distance) as tiebreak. This intentionally demotes the caller's `sort_mode` preference — curated POIs always lead the result set; sort_mode breaks ties inside the 'standard' tier. Pattern: drift 5.90 DROP-loop + bare `CREATE FUNCTION` (RETURNS shape changes; `CREATE OR REPLACE` would error). The 70-floor (addendum §2.1) was previously enforced nowhere in live runtime — only in the offline simulator (`scripts/simulate-trip/`) and curation export (`scripts/curation/export.ts`); G2 closes that gap at the RPC layer so all consumers benefit consistently.
- Applied live via `scripts/poi-import/apply-g2-floors.mjs` (H1.6 pattern; no local dev DB). Verification (LA→Mammoth 20mi corridor): 17-row seed confirmed; 25 `priority_tier='curator'` POIs surface (out of 26 editorial seeds along the route); 0 sub-90 architecture leaks; total post-floor count = 42 (25 curator + 17 standard) vs unbounded pre-G2.
- `lib/supabase.ts` POI interface gains `priority_tier?: 'curator' | 'iconic' | 'standard'` — additive optional union; existing `as POI[]` casts keep working since the interface is open.
- **Follow-up backlog (small, not in scope for the G2 commit):** `scripts/precache-popular-routes.ts`'s `--min-score N` CLI arg semantic changes from "fetch everything then drop below N" to "fetch above category floors then drop below N." Same surface for above-floor callers; below-floor callers see fewer rows. No action needed unless that script gets re-run; the script's TS would benefit from a `priority_tier`-aware SELECT projection on the next touch.

**Applied 2026-05-20 (C1 — RPC corridor extension for curator/iconic POIs):**
- 20260520000001 `rpc_pois_curator_corridor_extension` — extends both `get_corridor_pois` and `get_nearby_pois` so that `editorial_curated = TRUE` and `iconic_local = TRUE` POIs bypass the user-set spatial filter (`corridor_width_miles` for route queries, `radius_m` for point queries), capped at a **25mi visibility horizon** (`25 * 1609.34` m ≈ 40233.5 m). Standard tier remains bound by the user-set value. Same OR-pattern as G2's significance-floor bypass — spatial equivalent of that significance-tier bypass; same gate (`editorial_curated = TRUE OR iconic_local = TRUE`); same "curator override on user controls" philosophy applied to the WHERE-clause's spatial filter. 25mi is the curator's visibility heuristic — past ~25mi atmospheric haze hides most landmarks unless at altitude with clear sightlines; hardcoded as `25 * 1609.34` inline matching the existing `corridor_width_miles * 1609.34` style. Future config-table migration if tuning needed. Pattern: `CREATE OR REPLACE FUNCTION` (signatures + RETURNS unchanged from G2; only WHERE clause body changes — no drift-5.90 DROP-loop needed).
- Applied live via `scripts/poi-import/apply-c1-curator-corridor.mjs`. Verification on LA→Mammoth 20mi corridor: pre-C1 baseline 42 → post-C1 48 (+6 curator POIs in the 20-25mi bypass band). All 8 hardcoded assertions PASS: Jawbone Siphon (24.94mi curator) surfaces with `priority_tier='curator'`; Adventure City (21.80mi curator) surfaces; Korean Bell of Friendship (23.78mi standard) correctly absent (no bypass for standard tier); **Mt Whitney (25.02mi curator) correctly excluded by 32m past the cap** — precision test confirming 25mi is the cliff; Red Rock Canyon SP (28.68mi) correctly excluded; Vasquez Rocks surfaces via `get_nearby_pois` from point (34.60, -118.10) at 14.79mi with 5mi radius (curator bypass); Vasquez Rocks correctly absent from downtown LA (34.0522, -118.2437) at 30mi past cap.
...
**J1a-followups-deferred (2026-05-19):** Phase J1a-followups removed three more Trip Setup controls per curator's Expo walk-through — the Density `SegmentedTrio`, the Min relevance `LabeledSlider`, and the POI distance `LabeledSlider` (the same control as the Trip Setup corridor — Drive-page corridor slider is a separate later commit). The Pace axis was renamed to Detail in the Zustand store (v2 → v3 persist migration; `pace` field carried forward to `detail`; option identifiers `full_drive` / `light_touch` unchanged). The `saveTrip()` payload hardcodes `density: 'balanced'` and `minRelevance: 0` with inline comments because both `trips.density` (3-value CHECK) and `trips.min_relevance` (0–100 CHECK) are NOT NULL with DB defaults — same J1a depth pattern. `trips.poi_distance_m` has no CHECK and is dropped from the customize payload entirely; `SaveTripParams.poiDistanceM` is now optional and the INSERT spreads it conditionally so the DB DEFAULT (500) applies on omit. Follow-up migration bundles with the J1a-deferred set above: drop `trips.density` + drop `trips.min_relevance` + drop `trips.poi_distance_m` columns, remove the corresponding hardcoded literals from `app/customize.tsx`, and remove the `density` / `minRelevance` / `poiDistanceM` keys from `SaveTripParams` in `lib/supabase.ts`. `app/drive.tsx` still reads `filters.density` / `filters.minRelevance` / `filters.corridorMi` from the nav-param JSON (with `?? 'balanced'` / `?? 0` / `?? 1` safe fallbacks) — customize continues to forward all three with mode-aware defaults so the home → customize → drive curation chain stays consistent.
...
- **Deferred arcs awaiting follow-up prompts (2026-05-12 EOD):**
  ...
  - **Drift 5.41 (partially-resolved) — 28 of 29 pre-existing tsc errors remain.** Repo-wide `npx tsc --noEmit` failed with 29 errors across `admin/` (15, Next.js path-alias resolution), `app/drive.tsx:335` (2, removed `setStoryCount` call site), `lib/__tests__/routeBadges.test.ts` (9, `BadgeRoute` widened), `scripts/poi-import/lib/category-map.ts:27` (1, typo — **RESOLVED 2026-05-22 during GNIS arc**; was also a runtime crash blocking every poi-import command), `scripts/precache-popular-routes.ts:434` (1, type narrowing). The other 28 errors across 4 files still need cleanup. Cleanup arc on hold until a future session — does not block ongoing work but tooling like pre-commit type-gates can't run repo-wide until resolved.
...
Standing order landed in [docs/roadstory-narration-curation-addendum.md §3.4.1](docs/roadstory-narration-curation-addendum.md): when a trip begins with the user already inside one or more region polygons, region narrations do **not** fire at trip start. The runtime waits for sustained movement (GPS speed ≥ 5 mph for ≥ 5 consecutive seconds), then runs a 30-second settle timer from the moment movement is first detected. On timer expiry, the highest-tier containing region fires; **on tier ties, the smallest containing polygon wins** (e.g., SFV before its containing Transverse Ranges province — same most-specific-wins semantic as the existing `detect_regions_at_location` RPC); subsequent containing regions follow the existing ~20-min rate limit.
...
- **food_drink Bucket B trigger v2** — LANDED via `a86e493`. Migration `20260521000002_food_drink_bucket_b.sql` updates `recompute_narrative_modes(pois)` so all food_drink POIs route to `{local}` regardless of `editorial_curated`. Per-row `narrative_modes_override` escape hatch remains for Madonna-Inn-class iconic exceptions (deferred). Supersedes the prior §15.10 framing that food_drink would be Bucket C-promotable via `iconic_local` curation — food/drink in Soul mode is a category mismatch with the soul doctrine.
- **In-N-Out Museum iconic_local callout** — LANDED via `92c0b6e`. Baldwin Park location seeded as category=`history` (museum, not food_drink slug) with `narrative_modes_override = TRUE` locking to `{local}`. Exercises the override path for category-edge entries where the food_drink Bucket B carveout doesn't naturally apply.
- **Tier 2 iconic_local food_drink editorial seed** — LANDED via `804b6d2`. Nine California food landmarks via `scripts/poi-import/seeds/editorial-tier2-iconic-food-drink-2026-05-21.sql` (Philippe the Original, Musso & Frank Grill, Bob's Big Boy Burbank, The Apple Pan, Original Tommy's, Roscoe's Hollywood, Erick Schat's Bakkery, Tadich Grill, Swan Oyster Depot). All route to `{local}` via the food_drink Bucket B trigger on INSERT — no override needed. Corpus state moves food_drink rows from 14 → 23. Broader 50+ candidate pool deferred to v1.1 backlog "Iconic food import gap workstream."
- **`landmark` category slug** — LANDED via `0be4031`. Schema migration `20260521000005_landmark_category_slug.sql` inserts `{slug: 'landmark', display_name: 'Iconic Landmark', sort_order: 13}` into `public.poi_categories` for Tier 1 iconic POIs that don't fit nature/geology/history/food_drink/architecture. Routes via Bucket C (no Bucket A or B guard catches the slug); editorial-curated rows resolve to `{soul,local}` automatically via the existing `pois_narrative_modes_recompute` trigger.
- **Narration preview workflow** — LANDED via `7257d08`. New scaffolding at `scripts/narration-preview/`: Claude Sonnet synthesizes Tier 1 long-form Soul prose from canonical POI fields; Google Cloud TTS Chirp 3 HD renders Opus audio. Forked-prompt pattern — own `prompts/tier1-soul-full.ts` system prompt with PRESERVATION RULES + ALLOWED ADJUSTMENTS (cadence rewrite, not synthesis); does NOT inherit from `server/prompts/pois/narrator_b_family.js`. CLI flags: `--source <id>`, `--mode tier1|standard`, `--audience family|kids|unfiltered|local`, `--model <id>`, `--cost-ceiling <usd>`, `--dry-run`. v1.0 voice locked to narrator_b set 4 (currently `en-US-Chirp3-HD-Sadachbia` @ 1.0×) when `mode=tier1`. Output directory `output/` gitignored via new `.gitignore` rule + global `*.opus`. Cost ~$0.08 per preview (Sonnet 4.6 ~$0.024 Claude + Google TTS ~$0.069 for 500w narration). Delimiter-marked `<<<NARRATION>>>` output protocol (not JSON) — robust to verbatim quoted material with embedded double-quotes (the issue that surfaced + was fixed mid-run during the Madonna Inn first generation).
- **Tier 1 iconic_local editorial seed (4 California landmarks)** — LANDED via `d1d6c7f`. Seed `scripts/poi-import/seeds/editorial-tier1-iconic-landmarks-2026-05-21.sql` inserts Madonna Inn, Salvation Mountain, Cabazon Dinosaurs, Roy's Motel & Café — all category=`landmark`, `source_type='editorial'`, `editorial_curated=TRUE`, `iconic_local=TRUE`, `significance_score=9000`, `intrinsic_depth='long'`. First end-to-end INSERT-path validation of landmark slug + Bucket C dual-mode routing — all 4 routed to `{soul,local}` with no override required. Roy's coords resolved via Wikipedia infobox (34.558982, -115.743917); Nominatim cross-check at National Trails Highway centerline drifted ~500m west (road centerline vs. building complex) — used Wikipedia per landmark-POI convention.
