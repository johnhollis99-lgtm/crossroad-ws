...
3. Food_drink `iconic_local` curation pass (foundational Local catalog seed) — **first concrete delivery under the §15.10 framework**, not a stand-alone concern. The food_drink floor=999 sentinel (commit `8b49c80`) was the per-category precursor pattern that the Editorial Gate later generalized for Soul-access control. Per the v2 amendment (commit `a86e493`), food_drink itself is now Bucket B (always `{local}`, no editorial-promotion to Soul) — the curation pass produces Tier 2 callout content routing through the Bucket B carveout on INSERT, with the per-row `narrative_modes_override` path available for category-edge cases (commit `92c0b6e`, In-N-Out Museum at category=`history`). See §15.10 for full delivery context.
