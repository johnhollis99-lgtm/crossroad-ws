...
import { ssmlize, stripMarkersAndTags, tallyMarkers } from '../server/lib/ssml.js';
...
// ── Config ─────────────────────────────────────────────────────────────────
// Migration Batch 2 (Track C, 2026-05-22): AUDIENCE_MODE retired in favor
// of narrator_slug + voice_slot per addendum §5 / Batch 1 narrator-keyed
// registry. Storage filename follows `{narrator_slug}_v{voice_slot}_{depth}`;
// voice_slot pinned to 1 here for determinism until the voice-slot picker
// UI lands.
...
async function main(): Promise<void> {
  ...
  // Template lookup — Batch 1 narrator-keyed registry. Signature is
  // (narratorSlug, depth, poi, sources) and returns an Anthropic messages
  // array; the prior {systemPrompt, buildUserPrompt} indirection is gone.
  ...
  // canonical key; voice_configs.mode column is dropped by Track D
  // migration 20260522000010.
  ...
  for (let i = 0; i < pois.length; i++) {
    ...
    try {
      ...
      // from the existing description / citation fields for the Batch-1
      // narrator-keyed template (poi, depth, sources) schema.
